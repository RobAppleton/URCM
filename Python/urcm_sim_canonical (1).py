# urcm_sim_canonical.py – URCM OSequence Simulation with Canonical PNRC and Extended Diagnostics
import numpy as np
import pandas as pd
...
def main():
    ...
    plt.plot(x, df['PNRC'], label='PNRC (zero-centered)')
    ...
    plt.plot(x, df['ΔSₑ'], label='Δ Entropy per Cycle', color='purple')
    plt.axhline(0, linestyle='dashed', color='gray')
    plt.title('Symbolic Entropy Change per Cycle (ΔSₑ)')
    plt.xlabel('Cycle')
    plt.ylabel('ΔSₑ')
    plt.legend()
    plt.grid(True)
    ...
    plt.figure(figsize=(12, 4))
    plt.plot(x, df['Ĉ_fix'], label='Ĉ_fix Active', color='orange', drawstyle='steps-post')
    plt.plot(x, df['Sₑ'], label='Entropy Sₑ', linestyle='dotted', color='blue')
    plt.title('Ĉ_fix Activity and Entropy')
    plt.xlabel('Cycle')
